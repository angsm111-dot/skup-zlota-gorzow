...
function renderCatalogManager(){
  ...
  if(!section.hidden){const noun=kind==='coins'?'Monety':'Sztabki';section.querySelector('#catalog-title').textContent=noun;section.querySelector('#catalog-toggle').textContent=`+ Dodaj ${kind==='coins'?'monetę':'sztabkę'}`}
  if(!section.hidden)renderCatalogList();
  decorateProductRows();
}

function renderCatalogList(){
  const container=document.querySelector('#catalog-own-list'),list=currentProducts();if(!container)return;
  if(!list.length){container.innerHTML='<p class="catalog-empty">Brak produktów w tej sekcji.</p>';return}
  container.innerHTML=list.map((product,index)=>`<article><div class="catalog-list-photo">${product.imageData?`<img src="${product.imageData}" alt="">`:'<span>bez zdjęcia</span>'}</div><div><strong>${product.name}</strong><small>Próba ${String(product.purity).replace('.',',')} • ${formatWeight(product.grossWeight)}</small></div><div class="catalog-list-order"><button type="button" data-list-move="${product.id}" data-direction="up" ${index===0?'disabled':''}>↑</button><button type="button" data-list-move="${product.id}" data-direction="down" ${index===list.length-1?'disabled':''}>↓</button></div><button type="button" data-list-edit="${product.id}">Edytuj</button></article>`).join('');
  container.querySelectorAll('[data-list-edit]').forEach(button=>button.onclick=()=>openCatalogForm(currentProducts().find(item=>item.id===button.dataset.listEdit)));
  container.querySelectorAll('[data-list-move]').forEach(button=>button.onclick=()=>moveCatalogProduct(button.dataset.listMove,button.dataset.direction));
}
...
function moveCatalogProduct(id,direction){
  const list=currentProducts(),from=list.findIndex(product=>product.id===id),to=direction==='up'?from-1:from+1;if(from<0||to<0||to>=list.length)return;
  [list[from],list[to]]=[list[to],list[from]];config.customProducts||=[];
  list.forEach((product,order)=>{const override=catalogOverride(product,order),index=config.customProducts.findIndex(item=>item.id===product.id);index>=0?config.customProducts[index]={...config.customProducts[index],...override}:config.customProducts.push(override)});
  markDirty();document.querySelector('#status').textContent='Zmieniono kolejność. Opublikuj cennik, aby ją zapisać.';render();
}
...
function openCatalogForm(product=null){
  ...
  document.querySelector('#catalog-image-note').textContent=catalogImageData?'Zdjęcie zapisane':catalogEditingBuiltIn?'Pozostanie obecne zdjęcie katalogowe':'Brak zdjęcia — można zapisać';
  document.querySelector('#catalog-image-help').textContent=catalogEditingBuiltIn?'Nowe zdjęcie jest opcjonalne. Bez niego pozostanie obecne.':'Zdjęcie jest opcjonalne i można je dodać później';
...
function saveCatalogProduct(event){
  event.preventDefault();const message=document.querySelector('#catalog-message');
  const metal=document.querySelector('#catalog-metal').value,productKind=document.querySelector('#catalog-kind').value,purity=Number(document.querySelector('#catalog-purity').value),grossWeight=Number(document.querySelector('#catalog-gross').value),fineWeight=Number(document.querySelector('#catalog-fine').value)||grossWeight*purity/1000,id=catalogEditId||`custom-${metal}-${productKind}-${Date.now()}`,existing=currentProducts().find(item=>item.id===id),product={id,name:document.querySelector('#catalog-name').value.trim(),description:document.querySelector('#catalog-description').value.trim(),metal,kind:productKind,purity,grossWeight,fineWeight,imageData:catalogImageData,imageKey:existing?.imageKey||'',imageIndex:existing?.imageIndex||0,order:existing?.order??currentProducts().length,active:document.querySelector('#catalog-active').checked,custom:!catalogEditingBuiltIn,overridden:catalogEditingBuiltIn,defaultMargin:Number(document.querySelector('#catalog-margin').value)||0};
