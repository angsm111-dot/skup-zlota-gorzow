...
function renderProductCalculator(){
  ...
  if(selected){const priceBox=view.querySelector('.product-facts span:nth-child(2) b');if(priceBox)priceBox.innerHTML=productMoneyPair(productPrice(selected))}
...
const renderItemsProductsPln=renderItems;
renderItems=function(){renderItemsProductsPln();document.querySelectorAll('#calc-items .calc-item').forEach((row,index)=>{const item=items[index],product=item.type==='product'?(item.product||Object.values(productGroups).flat().find(value=>value.id===item.productId)):null,unit=product?productPrice(product):buyPrice(item.metal,item.purity),amount=product?item.quantity*unit:item.weight*unit,small=row.querySelector('small'),oldAmount=row.querySelector(':scope > b');if(small)small.innerHTML=product?`${item.quantity} szt. × ${productMoneyPair(unit)} • próba ${String(product.purity).replace('.',',')}`:`${item.weight.toFixed(2).replace('.',',')} g × ${productMoneyPair(unit)}`;if(oldAmount)oldAmount.outerHTML=productMoneyPair(amount)});const total=items.reduce((sum,item)=>item.type==='product'?sum+item.quantity*productPrice(item.product):sum+item.weight*buyPrice(item.metal,item.purity),0);document.querySelector('#total-value').innerHTML=productMoneyPair(total)};
...
function updateProductConditionNotes(){
  ...
  document.querySelectorAll('.product-card').forEach((card,index)=>{
    ...
    const label=coin?'cena skupu za monetę w stanie menniczym':'cena skupu za sztukę';
