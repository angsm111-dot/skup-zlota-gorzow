...
document.querySelectorAll('[data-footer-metal]').forEach(link=>link.addEventListener('click',()=>{activeMetal=link.dataset.footerMetal;zoomRange=null;document.querySelectorAll('[data-metal-tab]').forEach(x=>x.classList.toggle('active',x.dataset.metalTab===activeMetal));document.querySelector('#chart-metal').value=activeMetal;renderPriceTable()}));
